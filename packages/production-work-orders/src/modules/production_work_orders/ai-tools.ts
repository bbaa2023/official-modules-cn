...
const listTool: AiToolDefinition = defineAiTool({
  ...
  async handler(input, context) {
    ...
    const parsed = z.object({ status: statusSchema.optional(), limit: z.number().int().min(1).max(100).default(20) }).parse(input)
...
const analysisTool: AiToolDefinition = defineAiTool({
  name: 'production_work_orders.analyze',
  displayName: 'Production — analyze execution',
  ...
  tags: ['read', 'analysis', 'production'],
  isMutation: false,
  requiredFeatures: ['production_work_orders.analyze'],
  inputSchema: z.object({ dueSoonDays: z.number().int().min(1).max(30).default(3), limit: z.number().int().min(1).max(100).default(50) }),
  async handler(input, context) {
    const scope = requireToolScope(context)
    const parsed = z.object({ dueSoonDays: z.number().int().min(1).max(30).default(3), limit: z.number().int().min(1).max(100).default(50) }).parse(input)
    const em = context.container.resolve<EntityManager>('em')
    const rows = await em.find(ProductionWorkOrder, { tenant_id: scope.tenantId, organization_id: scope.organizationId, deleted_at: null }, { limit: parsed.limit, orderBy: { due_date: 'asc' } })
    const now = new Date()
    const dueSoonUntil = new Date(now.getTime() + parsed.dueSoonDays * 24 * 60 * 60 * 1000)
    const active = rows.filter((row) => row.status === 'released' || row.status === 'in_progress')
...
const reportTool: AiToolDefinition = defineAiTool({
  name: 'production_work_orders.report',
  displayName: 'Production — report quantity',
  description: 'Propose a production quantity report. Persistence requires platform mutation approval.',
  tags: ['mutation', 'production'],
  isMutation: true,
  requiredFeatures: ['production_work_orders.report'],
  inputSchema: productionReportSchema,
  async handler(input, context) {
    requireToolScope(context)
    const parsed = productionReportSchema.parse(input)
    const runner = createAiApiOperationRunner(context as any)
    const { workOrderId, ...body } = parsed
    const response = await runner.run({ method: 'POST', path: `/production-work-orders/work-orders/${workOrderId}/report`, body: body as Record<string, unknown> })
    if (!response.success) throw new Error(response.error ?? 'Failed to report production quantity')
    return { recordId: workOrderId, result: response.data }
  },
})

const operationReportTool: AiToolDefinition = defineAiTool({
  name: 'production_work_orders.report_operation',
  displayName: 'Production — report operation quantity',
  description: 'Propose an operation quantity report. Persistence requires platform mutation approval.',
  tags: ['mutation', 'production'],
  isMutation: true,
  requiredFeatures: ['production_work_orders.execute'],
  inputSchema: operationReportSchema,
  async handler(input, context) {
    requireToolScope(context)
    const parsed = operationReportSchema.parse(input)
    const runner = createAiApiOperationRunner(context as any)
    const { workOrderId, operationId, ...body } = parsed
    const response = await runner.run({ method: 'POST', path: `/production-work-orders/work-orders/${workOrderId}/operations/${operationId}/report`, body: body as Record<string, unknown> })
    if (!response.success) throw new Error(response.error ?? 'Failed to report operation quantity')
    return { recordId: operationId, result: response.data }
  },
})

export const aiTools: AiToolDefinition[] = [listTool, getTool, analysisTool, createTool, updateTool, transitionTool, reportTool, operationReportTool]
